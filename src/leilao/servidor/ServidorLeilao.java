...
public class ServidorLeilao implements CoordenadorPrimario {
    ...
    public void iniciar() {
        try {
            ...
            DiscoveryMdns.anunciarComoPrimario(PORTA_CLIENTES, CONFIGURACAO.obterPortaHttp());
            enviarEstadoParaReplica(gerenciadorLeiloes.criarEstadoReplicado(repositorioUsuarios.exportarUsuarios()));
    ...
    public void replicarAposLanceAceito(int idLeilao, Lance lance) {
        ...
        boolean replicado = enviarEstadoParaReplica(gerenciadorLeiloes.criarEstadoReplicado(repositorioUsuarios.exportarUsuarios()));
    ...
    private synchronized boolean tentarConectarReplica() {
        ...
        // Tenta primeiro o endereço do config.
        // Se falhar, usa o discovery para encontrar qualquer réplica na rede.
        if (!tentarConexaoReplica(ENDERECO_REPLICA)) {
            ...
            if (encontrada != null) {
                return tentarConexaoReplica(encontrada.ip);
            }
    ...
    private boolean tentarConexaoReplica(String endereco) {
        try {
            socketReplicacao = new Socket();
            socketReplicacao.connect(
                    new java.net.InetSocketAddress(endereco, PORTA_REPLICACAO),
