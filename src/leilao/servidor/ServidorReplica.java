...
public class ServidorReplica implements CoordenadorPrimario {
    ...
    public void iniciar() {
        ...
        DiscoveryMdns.anunciarComoReplica(PORTA_REPLICACAO);
    ...
    private void processarEstadoRecebido(GerenciadorLeiloes.EstadoReplicado estado) {
        ...
        repositorioUsuarios.importarUsuarios(estado.obterUsuarios());
...
private synchronized void promoverParaPrimario() {
    ...
    // Passa a anunciar como primário para que clientes e novas réplicas
    // consigam encontrar este servidor via discovery após o failover.
    DiscoveryMdns.anunciarComoPrimario(PORTA_CLIENTES_APOS_PROMOCAO,
            ConfiguracaoRede.instancia().obterPortaHttp());
            ...
            try {
                ...
                for (Leilao leilao : gerenciadorLeiloes.obterTodosLeiloes().values()) {
                    if (!leilao.estaAtivo() && leiloesAnunciados.add(leilao.obterId())) {
                        ...
                        enviarEstadoParaReplicaRetorno(gerenciadorLeiloes.criarEstadoReplicado(repositorioUsuarios.exportarUsuarios()));
                    }
                }
            } catch (InterruptedException erro) {
                Thread.currentThread().interrupt();
    ...
    public void replicarAposLanceAceito(int idLeilao, Lance lance) {
        ...
        boolean replicado = enviarEstadoParaReplicaRetorno(
                gerenciadorLeiloes.criarEstadoReplicado(repositorioUsuarios.exportarUsuarios()));
    ...
    private synchronized boolean tentarConectarReplicaRetorno() {
        ...
        String enderecoRetorno = ENDERECO_REPLICA_RETORNO;

        // Tenta primeiro o endereço do config (ou localhost).
        // Se falhar, usa o discovery para encontrar qualquer réplica na rede.
        if (!tentarConexaoReplicaRetorno(enderecoRetorno)) {
            System.out.println("[INFO] Endereço fixo falhou. Tentando discovery...");
            DiscoveryMdns.ReplicaEncontrada encontrada = DiscoveryMdns.descobrirReplica();
            if (encontrada != null) {
                enderecoRetorno = encontrada.ip;
                return tentarConexaoReplicaRetorno(enderecoRetorno);
            }
            return false;
        }
    ...
    private boolean tentarConexaoReplicaRetorno(String enderecoRetorno) {
        try {
            socketReplicaRetorno = new Socket();
            socketReplicaRetorno.connect(
                    new java.net.InetSocketAddress(enderecoRetorno, PORTA_REPLICACAO),
            ...
            System.out.println("[INFO] Réplica de retorno conectada em "
                    + enderecoRetorno + ":" + PORTA_REPLICACAO + ".");
            return true;
        } catch (IOException erro) {
            fecharConexaoReplicaRetorno();
            return false;
