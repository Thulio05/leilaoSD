...
public class RepositorioUsuarios {
    ...
    /**
     * Exporta uma cópia do mapa de usuários para ser incluída no
     * EstadoReplicado e enviada à réplica junto com o estado dos leilões.
     */
    public synchronized Map<String, String> exportarUsuarios() {
        return new HashMap<>(usuarios);
    }

    /**
     * Importa usuários recebidos via replicação.
     * Chamado pela réplica ao restaurar o estado — garante que ela conhece
     * todos os usuários cadastrados no primário, inclusive os mais recentes.
     * Persiste também no arquivo local para sobreviver a reinicializações.
     */
    public synchronized void importarUsuarios(Map<String, String> mapaRecebido) {
        if (mapaRecebido == null || mapaRecebido.isEmpty()) {
            return;
        }
        usuarios.putAll(mapaRecebido);
        reescreverArquivo();
        System.out.println("[INFO] " + mapaRecebido.size()
                + " usuário(s) sincronizado(s) via replicação.");
    }
    ...
    private void reescreverArquivo() {
        try (FileWriter escritor = new FileWriter(caminhoArquivo, false)) {
            ...
            System.out.println("[ALERTA] Não foi possível persistir usuários: "
                    + erro.getMessage());
        }
    }
